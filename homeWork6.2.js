	...
	___ ЗАДАЧИ ПО МЕТОДАМ МАССИВОВ ___
	...
	*/
	...
	function camelize(str) {
	  ...
	  arr = arr.map((item, index) => index == 0 ? item : item[0].toUpperCase() + item.slice(1)).join('');
	...
	*/
	...
	filterRange([5, 3, 8, 1], 1, 4); // [5, 3, 8, 1] - исходный массив  [3, 1] - измененный массив
	...
	*/
	...
	filterRangeInPlace([5, 3, 8, 1], 1, 4); // [3, 1]
	
	
	// в книге предлагается такое решение, но я не совсем понимаю зачем
	// мой вариант так же изменяет исходный массив и, если нужно, может ничего не возвращать
	// ли я не прав?
	...
	*/
	...
	function sortArrayInDisentingOrder(initialArray) {
	  initialArray.sort().reverse();
	  	  
	  return initialArray;
	  
	}

	sortArrayInDisentingOrder([5, 2, 1, -10, 8]); // [8, 5, 2, 1, -10]
	.____________________________
